...
public class BezierCurve : MonoBehaviour {
    ...
    public void DrawBezierLine () {
        lineR = GameObject.Find("RodEnd").GetComponent<LineRenderer>();

        Gizmos.color = drawColor;
		
		if(points.Length > 1){
			for(int i = 0; i < points.Length - 1; i++){
                DrawCurve(lineR, points[i], points[i+1], resolution);
			}
			
            if (close) DrawCurve(lineR, points[points.Length - 1], points[0], resolution);
		}
	}
    ...
    //!!!!! Muista ! Siiman ongelma play modess ja buildissa johtuu siitä että bezier piirretään editoriin !!!!!!
    ...
    /// 
    /// 
    /// !!!!!Henrik täältä löytyy osuus mitä muokata !!!!!!
    /// 
    /// 
    /// 
    public static void DrawCurve(LineRenderer lineR, BezierPoint p1, BezierPoint p2, int resolution)
	{
		int limit = resolution+1;
		float _res = resolution;
		Vector3 lastPoint = p1.position;
		Vector3 currentPoint = Vector3.zero;
        lineR.positionCount = limit;
		
		for(int i = 1; i < limit; i++){
			currentPoint = GetPoint(p1, p2, i/_res);
            lineR.SetPosition(i, currentPoint);
            //print(i + " is set");
